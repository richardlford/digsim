...
Module Info.
  ...
  Definition source_file := "state_list.c"%string.
        ...
        (Ssequence
          (Sassign
            (Efield
              (Ederef
                (Etempvar _result (tptr (Tstruct _s_state_list noattr)))
                (Tstruct _s_state_list noattr)) _next
              (tptr (Tstruct _s_state_list noattr)))
            (Etempvar _next (tptr (Tstruct _s_state_list noattr))))
          (Sreturn (Some (Etempvar _result (tptr (Tstruct _s_state_list noattr))))))))))
|}.

Definition f_state_list_cons := {|
  fn_return := (tptr (Tstruct _s_state_list noattr));
  fn_callconv := cc_default;
...
Definition f_reverse_state_list := {|
  fn_return := (tptr (Tstruct _s_state_list noattr));
  fn_callconv := cc_default;
  fn_params := ((_list, (tptr (Tstruct _s_state_list noattr))) :: nil);
  fn_vars := nil;
  fn_temps := ((_v, (tptr (Tstruct _s_state_list noattr))) ::
               (_w, (tptr (Tstruct _s_state_list noattr))) ::
               (_t, (tptr (Tstruct _s_state_list noattr))) :: nil);
  fn_body :=
(Ssequence
  (Sset _v (Etempvar _list (tptr (Tstruct _s_state_list noattr))))
  (Ssequence
    (Sset _w (Ecast (Econst_int (Int.repr 0) tint) (tptr tvoid)))
    (Ssequence
      (Swhile
        (Ebinop One (Etempvar _v (tptr (Tstruct _s_state_list noattr)))
          (Ecast (Econst_int (Int.repr 0) tint) (tptr tvoid)) tint)
        (Ssequence
          (Sset _t
            (Efield
              (Ederef (Etempvar _v (tptr (Tstruct _s_state_list noattr)))
                (Tstruct _s_state_list noattr)) _next
              (tptr (Tstruct _s_state_list noattr))))
          (Ssequence
            (Sassign
              (Efield
                (Ederef (Etempvar _v (tptr (Tstruct _s_state_list noattr)))
                  (Tstruct _s_state_list noattr)) _next
                (tptr (Tstruct _s_state_list noattr)))
              (Etempvar _w (tptr (Tstruct _s_state_list noattr))))
            (Ssequence
              (Sset _w (Etempvar _v (tptr (Tstruct _s_state_list noattr))))
              (Sset _v (Etempvar _t (tptr (Tstruct _s_state_list noattr))))))))
      (Sreturn (Some (Etempvar _w (tptr (Tstruct _s_state_list noattr))))))))
|}.
