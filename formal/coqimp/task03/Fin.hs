...
rect2 :: (Prelude.Integer -> a1) -> (Prelude.Integer -> Coq_t -> a1) ->
         (Prelude.Integer -> Coq_t -> a1) -> (Prelude.Integer -> Coq_t -> Coq_t ->
         a1 -> a1) -> Prelude.Integer -> Coq_t -> Coq_t -> a1
rect2 h0 h1 h2 hS _ a b =
  case a of {
   F1 m -> caseS' m b (h0 m) (h1 m);
   FS m a' -> caseS' m b (h2 m a') (\b' -> hS m a' b' (rect2 h0 h1 h2 hS m a' b'))}
...
weak :: Prelude.Integer -> Prelude.Integer -> Prelude.Integer -> (Coq_t -> Coq_t) ->
        Coq_t -> Coq_t
weak m n p f =
  (\fO fS n -> if n Prelude.== 0 then fO () else fS (n Prelude.- 1))
    (\_ -> f)
