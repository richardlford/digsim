...
align n amount =
  (Prelude.*)
    (BinInt._Z__div ((Prelude.-) ((Prelude.+) n amount) ((\x -> x) 1)) amount)
    amount

option_eq :: (a1 -> a1 -> Prelude.Bool) -> (Prelude.Maybe a1) -> (Prelude.Maybe 
             a1) -> Prelude.Bool
option_eq eqA x y =
  Datatypes.option_rect (\a x0 ->
    case x0 of {
     Prelude.Just a0 ->
      Specif.sumbool_rec (\_ -> Prelude.True) (\_ -> Prelude.False) (eqA a a0);
...
list_disjoint_dec :: (a1 -> a1 -> Prelude.Bool) -> (([]) a1) -> (([]) a1) ->
                     Prelude.Bool
