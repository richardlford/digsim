  ...
  let (ev', result_sim) =
    ...
    then let (sim2, new_time_opt) = handle_event model_handlers ev sim_log in
         ...
         (match new_time_opt with
          | Some new_time ->
            ((set time (fun f e -> { key = e.key; time = (f e.time) }) (constructor new_time) ev), sim2log)
          | None -> (ev, sim2log))
    else (ev, sim_log)
  in
  let result_log =
...
let rec min_event_time evs min_so_far =
  ...
  | ev :: evtl ->
    let et = ev.time in
    let new_min = if (&&) (Float.cmp Cgt et Float.zero) (Float.cmp Clt et min_so_far) then et else min_so_far in
...
let advance_model sim =
  ...
  let rounded_new_t = Float.div (FloatIO.round (Float.mul new_t tenTo6)) tenTo6 in
...
let run_sim sim =
  ...
  log_sim ('r'::('u'::('n'::('_'::('s'::('i'::('m'::(':'::('r'::('e'::('s'::('u'::('l'::('t'::[]))))))))))))))
    result_sim
...
let sim_in _ =
  ...
  log_sim ('s'::('i'::('m'::('_'::('i'::('n'::(':'::('s'::('i'::('m'::('2'::[]))))))))))) sim2
