...
(** val coq_Bopp : coq_Z -> coq_Z -> (bool -> nan_pl -> bool * nan_pl) -> binary_float -> binary_float **)
...
(** val coq_Babs : coq_Z -> coq_Z -> (bool -> nan_pl -> bool * nan_pl) -> binary_float -> binary_float **)
...
(** val coq_Bcompare : coq_Z -> coq_Z -> binary_float -> binary_float -> comparison option **)
...
(** val shr_fexp : coq_Z -> coq_Z -> coq_Z -> coq_Z -> location -> shr_record * coq_Z **)

let shr_fexp prec emax =
  let emin = Z.sub (Z.sub (Zpos (Coq_xI Coq_xH)) emax) prec in
  let fexp = coq_FLT_exp emin prec in
  (fun m e l -> shr (shr_record_of_loc m l) e (Z.sub (fexp (Z.add (coq_Zdigits2 m) e)) e))
...
(** val binary_round_aux : coq_Z -> coq_Z -> mode -> bool -> positive -> coq_Z -> location -> full_float **)

let binary_round_aux prec emax mode0 sx mx ex lx =
  let (mrs', e') = shr_fexp prec emax (Zpos mx) ex lx in
  let (mrs'', e'') =
    shr_fexp prec emax (choice_mode mode0 sx mrs'.shr_m (loc_of_shr_record mrs')) e' Coq_loc_Exact
  in
...
(** val coq_Bmult :
    coq_Z -> coq_Z -> (binary_float -> binary_float -> bool * nan_pl) -> mode -> binary_float -> binary_float
    -> binary_float **)
   ...
   | B754_finite (sx, mx, ex) ->
     (match y with
      ...
      | B754_finite (sy, my, ey) ->
        coq_FF2B prec emax
          (binary_round_aux prec emax m (xorb sx sy) (Pos.mul mx my) (Z.add ex ey) Coq_loc_Exact)))
...
let shl_align_fexp prec emax =
  let emin = Z.sub (Z.sub (Zpos (Coq_xI Coq_xH)) emax) prec in
  let fexp = coq_FLT_exp emin prec in (fun mx ex -> shl_align mx ex (fexp (Z.add (Zpos (digits2_pos mx)) ex)))

(** val binary_round : coq_Z -> coq_Z -> mode -> bool -> positive -> coq_Z -> full_float **)
...
(** val binary_normalize : coq_Z -> coq_Z -> mode -> coq_Z -> coq_Z -> bool -> binary_float **)
...
let coq_Bminus prec emax minus_nan m x y =
  coq_Bplus prec emax minus_nan m x (coq_Bopp prec emax (fun x0 x1 -> (x0, x1)) y)

(** val coq_Fdiv_core_binary : coq_Z -> coq_Z -> coq_Z -> coq_Z -> coq_Z -> (coq_Z * coq_Z) * location **)
...
let coq_Fdiv_core_binary prec m1 e1 m2 e2 =
  ...
  (match Z.sub (Z.add d2 prec) d1 with
   | Zpos p ->
     ...
     let (q, r) = coq_Zfast_div_eucl m m2 in ((q, e'), (new_location m2 r Coq_loc_Exact))
   | _ -> let (q, r) = coq_Zfast_div_eucl m1 m2 in ((q, e), (new_location m2 r Coq_loc_Exact)))
  ...
  (match x with
   ...
   | B754_finite (sx, mx, ex) ->
     (match y with
      ...
      | B754_finite (sy, my, ey) ->
        coq_FF2B prec emax
          (let (p, lz) = coq_Fdiv_core_binary prec (Zpos mx) ex (Zpos my) ey in
           let (mz, ez) = p in
           (match mz with
