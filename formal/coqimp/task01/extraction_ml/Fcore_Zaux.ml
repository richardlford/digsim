...
| Coq_xO b' ->
  (match a with
   | Coq_xI a' ->
     let (q, r) = coq_Zpos_div_eucl_aux1 a' b' in (q, (Z.add (Z.mul (Zpos (Coq_xO Coq_xH)) r) (Zpos Coq_xH)))
   | Coq_xO a' -> let (q, r) = coq_Zpos_div_eucl_aux1 a' b' in (q, (Z.mul (Zpos (Coq_xO Coq_xH)) r))
   | Coq_xH -> (Z0, (Zpos a)))
| Coq_xH -> ((Zpos a), Z0)
