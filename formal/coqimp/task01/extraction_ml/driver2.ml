...
(** val updatePosToStateVarTree :
    (stateVar * char list) list -> posToStateVarTreeType -> posToStateVarTreeType **)

let rec updatePosToStateVarTree svIdStrList tree =
  match svIdStrList with
  | [] -> tree
  | p :: xtail ->
    let (sv, _) = p in let tree1 = PTree.set (svIndex sv) sv tree in updatePosToStateVarTree xtail tree1
...
let rec updateFloatSvTree valList intree =
  match valList with
  | [] -> intree
  | p :: vltail -> let (sv, fval) = p in let tree1 = SvTree.set sv fval intree in updateFloatSvTree vltail tree1
...
let set_var key0 val0 sim =
  ...
  in
  log_sim ('s'::('e'::('t'::('_'::('v'::('a'::('r'::(':'::('r'::('e'::('s'::('u'::('l'::('t'::[]))))))))))))))
    result_sim
...
(** val handle_event :
    (char list * event_function_signature) list -> eventTy -> simTy -> simTy * float option **)
...
let rec handle_event handlers this sim =
  ...
  | p :: otherHandlers ->
    ...
    if eqb eventKey this.key then evFun this sim else handle_event otherHandlers this sim
