...
module FloatIO =
 struct
  ...
  | Zneg x -> Float.neg (Float.from_parsed (Coq_xO (Coq_xI (Coq_xO Coq_xH))) x Z0)
  ...
  module Details =
   struct
    ...
    let log10of2scaled3 =
      Zpos (Coq_xI (Coq_xO (Coq_xI (Coq_xI (Coq_xO (Coq_xI (Coq_xO (Coq_xO Coq_xH))))))))
 ...
 (fun f c ->
  ...
  f (h 0) (h 1) (h 2) (h 3) (h 4) (h 5) (h 6) (h 7))
    ...
    let strToFloat s =
      ...
      | Some p ->
        ...
        ret (Obj.magic optionM)
          (match zmant with
           | Z0 -> Float.zero
           | Zpos x -> Float.from_parsed (Coq_xO (Coq_xI (Coq_xO Coq_xH))) x adjustedZexp
           | Zneg x -> Float.neg (Float.from_parsed (Coq_xO (Coq_xI (Coq_xO Coq_xH))) x adjustedZexp))
      | None -> None
    ...
    let scale_exp d e =
      Z.sub (Z.sub d (Zpos (Coq_xO Coq_xH)))
        (Z.div (Z.mul (Z.sub (Z.add e precision_of_float) (Zpos Coq_xH)) log10of2scaled3) (Zpos (Coq_xO (Coq_xO
              ...
              (match num with
               ...
               | _ ->
                 append
                   ...
                   (digits (Z.modulo num (Zpos (Coq_xO (Coq_xI (Coq_xO Coq_xH))))))))
    ...
    let coq_Z_to_string_base10 min_digits num =
      if Z.ltb num Z0
      then append ('-'::[])
             (coq_Z_to_string_base10_aux min_digits (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S
           ...
           else coq_Z_to_string_base10_aux min_digits (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S
    ...
    let scaled_float_to_Z x fdigs =
      match Float.to_long (Float.mul x (Float.from_parsed (Coq_xO (Coq_xI (Coq_xO Coq_xH))) Coq_xH fdigs)) with
    ...
    let insert_decimal s fdigs =
      let len = length s in
      let front = sub len fdigs in append (substring O front s) (append ('.'::[]) (substring front fdigs s))
    ...
    let float_to_string_unsigned x fdigs =
      ...
      | B754_finite (b, _, e) ->
        ...
        then []
             ...
             let scale' = if PeanoNat.Nat.ltb lb10 fdigs then Z.add scale (Z.of_nat (sub fdigs lb10)) else scale
             in
             let b10' =
               if PeanoNat.Nat.ltb lb10 fdigs
               then let scaled' = scaled_float_to_Z x scale' in coq_Z_to_string_base10 fdigs scaled'
               else b10
             in
             let d10 = insert_decimal b10' digs_after_dec in
             let true_exp = Z.sub (Z.sub (Z.of_nat fdigs) scale') (Zpos Coq_xH) in
             let exp_string = coq_Z_to_string_base10 (S O) true_exp in append d10 (append ('e'::[]) exp_string)
      | _ -> []
    ...
    let pad_to_width width s =
      let ls = length s in let pads = repeat_string (sub width ls) (' '::[]) in append pads s
  ...
  let coq_ZofFloat f =
    match coq_ZofB (Zpos (Coq_xI (Coq_xO (Coq_xI (Coq_xO (Coq_xI Coq_xH)))))) (Zpos (Coq_xO (Coq_xO (Coq_xO
            (Coq_xO (Coq_xO (Coq_xO (Coq_xO (Coq_xO (Coq_xO (Coq_xO Coq_xH))))))))))) f with
    | Some z -> z
    | None -> Z0
