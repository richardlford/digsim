...
(** val schedule_event :
    eventTy list -> char list -> float -> eventTy list **)

let rec schedule_event evs evkey new_time =
  match evs with
  | [] -> []
  | ev :: evtl ->
    let ev' =
      if eqb ev.key evkey
      then set time (fun f e -> { key = e.key; time = (f e.time) })
             (constructor new_time) ev
      else ev
    in
