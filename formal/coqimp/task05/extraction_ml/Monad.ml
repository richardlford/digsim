...
type 'm coq_MonadOps = { bind : (__ -> __ -> 'm -> (__ -> 'm) -> 'm);
                         ret : (__ -> __ -> 'm) }
...
type 't coq_MonadTransformerOps = { coq_Tops : (__ -> __ coq_MonadOps -> 't
                                               coq_MonadOps);
                                    lift : (__ -> __ coq_MonadOps -> __ -> __
                                           -> 't) }

(** val coq_Tops :
    'a1 coq_MonadTransformerOps -> 'a2 coq_MonadOps -> 'a1 coq_MonadOps **)

let coq_Tops monadTransformerOps h =
  let { coq_Tops = tops; lift = _ } = monadTransformerOps in
  Obj.magic tops __ h
...
(** val monad_of_transformer :
    'a1 coq_MonadTransformerOps -> 'a1 coq_MonadOps **)
...
let optionT =
  { coq_Tops = (fun _ h -> { bind = (fun _ _ x f ->
    bind h x (fun a -> match a with
                       | Some a' -> f a'
                       | None -> ret h None)); ret = (fun _ x ->
    ret h (Some x)) }); lift = (fun _ h _ x ->
