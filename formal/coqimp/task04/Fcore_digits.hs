...
coq_Zdigit beta n k =
  BinInt._Z__rem
    (BinInt._Z__quot n (BinInt._Z__pow (Fcore_Zaux.radix_val beta) k))
    (Fcore_Zaux.radix_val beta)

coq_Zsum_digit :: Fcore_Zaux.Coq_radix -> (Prelude.Integer ->
...
coq_Zscale beta n k =
  case BinInt._Z__leb 0 k of {
   Prelude.True ->
    (Prelude.*) n (BinInt._Z__pow (Fcore_Zaux.radix_val beta) k);
   Prelude.False ->
    BinInt._Z__quot n
...
coq_Zdigits beta n =
  (\fO fP fN n -> if n Prelude.== 0 then fO () else
    ...
    coq_Zdigits_aux beta n ((\x -> x) 1) (Fcore_Zaux.radix_val beta)
      (digits2_Pnat p))
    (\p ->
    coq_Zdigits_aux beta ((\x -> x) p) ((\x -> x) 1)
      (Fcore_Zaux.radix_val beta) (digits2_Pnat p))
    n
