...
bpow r e =
  (\fO fP fN n -> if n Prelude.== 0 then fO () else
    ...
    Rdefinitions.coq_Rinv
      (coq_Z2R (BinInt._Z__pow_pos (Fcore_Zaux.radix_val r) p)))
    e
...
ln_beta r x =
  ...
  (Prelude.+)
    (coq_Zfloor
      (Rdefinitions.coq_Rdiv (Rpower.ln (Rbasic_fun.coq_Rabs x)) fact))
    ((\x -> x) 1)
...
coq_LPO_min =
  let {
   s = RIneq.coq_Rle_lt_dec Raxioms.completeness (Rdefinitions.coq_IZR 0)}
  in
  case s of {
   ...
   Prelude.False -> Prelude.Just
    (BinInt._Z__abs_nat
      ((Prelude.-)
        (Rdefinitions.up (Rdefinitions.coq_Rinv Raxioms.completeness))
        ((\x -> x) ((\x -> 2 Prelude.* x) 1))))}
