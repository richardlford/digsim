...
antisymmetry :: (Equivalence a1 a2) -> (Antisymmetric a1 a2 a3) -> a1 -> a1
                -> a3 -> a3 -> a2
...
flip_Equivalence h =
  case h of {
   Build_Equivalence equivalence_Reflexive0 equivalence_Symmetric0
    equivalence_Transitive0 -> Build_Equivalence equivalence_Reflexive0
    (\x y x0 -> equivalence_Symmetric0 y x x0) (\x y z x0 x1 ->
...
arrow_Transitive_obligation_1 :: (Coq_arrow a1 a2) -> (Coq_arrow a2 a3) ->
                                 Coq_arrow a1 a3
...
relation_equivalence_equivalence =
  Build_Equivalence (\_ _ _ -> (,) (\x -> x) (\x -> x)) (\_ _ x x0 y0 -> (,)
    (\x1 ->
    let {x2 = x x0} in
    let {x3 = x2 y0} in Datatypes.prod_rect (\_ b -> b x1) x3) (\x1 ->
    let {x2 = x x0} in
    let {x3 = x2 y0} in Datatypes.prod_rect (\a _ -> a x1) x3))
    (\_ _ _ x x0 x1 y0 ->
    let {x2 = x x1 y0} in
    let {x3 = x0 x1 y0} in
    Datatypes.prod_rect (\a b ->
      Datatypes.prod_rect (\a0 b0 -> (,) (\x4 -> let {x5 = a0 x4} in a x5)
        (\x4 -> let {x5 = b x4} in b0 x5)) x2) x3)

relation_implication_preorder :: PreOrder (Coq_crelation a1)
                                 (Coq_subrelation a1 Any Any)
relation_implication_preorder =
  Build_PreOrder (\_ _ _ x -> x) (\_ _ _ x x0 x1 y0 x2 ->
    x0 x1 y0 (x x1 y0 x2))
...
partial_order_equivalence :: (Equivalence a1 a2) -> (PreOrder a1 a3) ->
                             (PartialOrder a1 a2 a3) ->
                             Coq_relation_equivalence a1 a2
                             (Coq_relation_conjunction a1 a3 a3)
partial_order_equivalence _ _ partialOrder =
  partialOrder

partial_order_antisym :: (Equivalence a1 a2) -> (PreOrder a1 a3) ->
                         (PartialOrder a1 a2 a3) -> Antisymmetric a1 
                         a2 a3
partial_order_antisym _ _ h x y x0 x1 =
  case h x y of {
   (,) _ x2 -> x2 ((,) x0 x1)}

coq_PartialOrder_inverse :: (Equivalence a1 a2) -> (PreOrder a1 a3) ->
                            (PartialOrder a1 a2 a3) -> PartialOrder a1 
                            a2 a3
coq_PartialOrder_inverse equ _ h x y =
  (,) (\x0 ->
    case h y x of {
     (,) x1 _ -> x1 (symmetry (coq_Equivalence_Symmetric equ) x y x0)})
    (\x0 ->
    case x0 of {
     (,) h1 h2 -> case h x y of {
                   (,) _ x1 -> x1 ((,) h2 h1)}})
