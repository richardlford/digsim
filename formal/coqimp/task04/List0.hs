...
destruct_list l =
  Datatypes.list_rect Prelude.Nothing (\a tail _ -> Prelude.Just
     ...
     (:) a0 l1 ->
      ...
      case s of {
       Specif.Coq_existT l' s0 ->
        Logic.eq_rect_r (Datatypes.app l' ((:) s0 ([]))) (Specif.Coq_existT
          ((:) a l') s0) ((:) a0 l1)}}) l __
    ...
    case x0 of {
     ...
     (:) a0 l1 ->
      Specif.sumbool_rec (\_ ->
        Specif.sumbool_rec (\_ -> Prelude.True) (\_ -> Prelude.False) (x l1))
        (\_ -> Prelude.False) (eq_dec a a0)}) l l'
...
list_power l l' =
  case l of {
   ([]) -> (:) ([]) ([]);
   (:) x t ->
    flat_map (\f -> map (\y -> (:) ((,) x y) f) l') (list_power t l')}
