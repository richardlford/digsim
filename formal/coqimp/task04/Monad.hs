...
data MonadTransformerOps t =
   Build_MonadTransformerOps (() -> (MonadOps Any) -> MonadOps t) (() ->
                                                                  (MonadOps
                                                                  Any) -> ()
                                                                  -> Any ->
                                                                  t)
...
lift :: (MonadTransformerOps a1) -> (MonadOps a2) -> a2 -> a1
lift monadTransformerOps h x =
  case monadTransformerOps of {
   Build_MonadTransformerOps _ lift0 -> unsafeCoerce lift0 __ h __ x}

idM :: MonadOps Any
idM =
  Build_MonadOps (\_ _ x f -> f x) (\_ x -> x)

idT :: MonadTransformerOps Any
idT =
  Build_MonadTransformerOps (\_ h -> h) (\_ _ _ x -> x)
...
optionT =
  Build_MonadTransformerOps (\_ h -> Build_MonadOps (\_ _ x f ->
    bind h x (\a ->
      case a of {
       Prelude.Just a' -> f a';
       Prelude.Nothing -> ret h Prelude.Nothing})) (\_ x ->
    ret h (Prelude.Just x))) (\_ h _ x ->
    bind h x (\a -> ret h (Prelude.Just a)))

optionM :: MonadOps (Prelude.Maybe Any)
optionM =
  monad_of_transformer (unsafeCoerce optionT)

liftA :: (MonadTransformerOps a1) -> (MonadOps a2) -> (a2 -> a2) -> a1 -> a1
liftA h h0 f x =
  bind (coq_Tops h h0) x (\y -> lift h h0 (f (ret h0 y)))
