...
scaled_mantissa :: Fcore_Zaux.Coq_radix -> (Prelude.Integer ->
...
round :: Fcore_Zaux.Coq_radix -> (Prelude.Integer -> Prelude.Integer) ->
         (Rdefinitions.R -> Prelude.Integer) -> Rdefinitions.R ->
         Rdefinitions.R
round beta fexp rnd x =
  \x -> throw false beta (Fcore_defs.Float
    (rnd (scaled_mantissa beta fexp x)) (canonic_exp beta fexp x))
...
coq_Znearest choice x =
  case Fcore_Raux.coq_Rcompare
         (Rdefinitions.coq_Rminus x
           (Fcore_Raux.coq_Z2R (Fcore_Raux.coq_Zfloor x)))
